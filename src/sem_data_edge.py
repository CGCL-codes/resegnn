...
class SematicTrainDataset2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tail_edge_seq, rel_edge_seq = None, None
        if 'edge' in self.history_method:
            all_tail_edge_seq = sp.load_npz(
                os.path.join(self.filepath, 'tail_edge_history_{}.npz'.format(self.train_times[index])))
            all_rel_edge_seq = sp.load_npz(
                os.path.join(self.filepath, 'rel_edge_history_{}.npz'.format(self.train_times[index])))
            tail_edge_seq = scipy_sparse_mat_to_torch_sparse_tensor(all_tail_edge_seq[seq_idx])
            rel_edge_seq = scipy_sparse_mat_to_torch_sparse_tensor(all_rel_edge_seq[rel_seq_idx])
        return input_list, start_history, output, tail_seq, rel_seq, tail_edge_seq, rel_edge_seq

    def __len__(self):
        return len(self.train_list)


class SematicTestDataset2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tail_edge_seq, rel_edge_seq = None, None
        if 'edge' in self.history_method:
            all_tail_edge_seq = sp.load_npz(os.path.join(self.filepath, 'tail_edge_history_{}.npz'.format(history_index)))
            tail_edge_seq = scipy_sparse_mat_to_torch_sparse_tensor(all_tail_edge_seq[seq_idx])
            all_rel_edge_seq = sp.load_npz(os.path.join(self.filepath, 'rel_edge_history_{}.npz'.format(history_index)))
            rel_edge_seq = scipy_sparse_mat_to_torch_sparse_tensor(all_rel_edge_seq[rel_seq_idx])
        return test_snap, test_triples_input, tail_seq, rel_seq, tail_edge_seq, rel_edge_seq
