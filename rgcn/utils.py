...
def r2e(triplets, num_rels):
    ...
    -------
    uniq_r:去重关系列表
    r_len:每个uniq_r中关系相连的实体在e_idx中的范围索引
    e_idx:实体索引列表
    """
...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu):
    """
    :param num_nodes: number of relation
    :param num_rels: number of relation
    :param triples: triples to build graph
    :return:
    """
    ...
    # src: relabeled src id
    # rel: original rel id
    # dst: relabeled dst id
    triples = triples[:, :3]
    ...
    # 去重关系列表 每个关系中相应实体在r_to_e中的索引 实体索引列表
...
def stat_ranks(rank_list):
    ...
    hits_value = [torch.mean((total_rank <= hit).float()) for hit in hits]
    return mrr,hits_value
...
def load_data(dataset, bfs_level=3, relabel=False):
    ...
    elif dataset in ['ICEWS18', 'ICEWS18dev', 'ICEWS14', "GDELT", "SMALL", "ICEWS14s", "ICEWS05-15", "YAGO",
                     "WIKI"]:
...
def getRelEntCooccurrence(quadruples, num_r):
